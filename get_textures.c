...
void get_textures(char *line, t_index *index, t_data *data, t_data_pars *pars)
{
	if (!ft_strncmp(line, "NO ", 3))
	{
		...
		data->my_mlx.addr_n = mlx_get_data_addr(pars->no, &data->my_mlx.bpp_n, &data->my_mlx.l_len_n, &data->my_mlx.endian_n);
	}
	else if (!ft_strncmp(line, "SO ", 3))
	{
		pars->so = textures(line, data);
		data->my_mlx.addr_s = mlx_get_data_addr(pars->so, &data->my_mlx.bpp_s, &data->my_mlx.l_len_s, &data->my_mlx.endian_s);
	}
	else if (!ft_strncmp(line, "EA ", 3))
	{
		pars->ea = textures(line, data);
		data->my_mlx.addr_e = mlx_get_data_addr(pars->ea, &data->my_mlx.bpp_e, &data->my_mlx.l_len_e, &data->my_mlx.endian_e);
	}
